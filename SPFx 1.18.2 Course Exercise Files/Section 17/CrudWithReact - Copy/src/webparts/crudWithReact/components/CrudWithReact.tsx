...
//   SPHttpClientBatch } from '@microsoft/sp-http';
...
export default class CrudWithReact extends React.Component<ICrudWithReactProps, ICrudWithReactState> {
  ...
  public createAndLoadInOneGo(): Promise<ISoftwareListItem[]> {
    let promise: Promise<ISoftwareListItem[]> = new Promise<ISoftwareListItem[]>((resolve, reject) => {
  

        
          const clientBatch: SPHttpClientBatch = this.props.context.spHttpClient.beginBatch();

         
          const batchOperations: Promise<ISoftwareListItem | ISoftwareListItem[]>[] = [
  ...
  public btnAddAndLoadInOneGo_click(): void {
    ...
    this.createAndLoadInOneGo()
    .then((softwareListItems: ISoftwareListItem[]) => {
      this.setState({ SoftwareListItems: softwareListItems,status: "loaded"});
    });
    
  }

  private _getListItemsAsBatch(clientBatch: SPHttpClientBatch): Promise<ISoftwareListItem[]> {
    
    const url: string = this.props.siteUrl + "/_api/web/lists/getbytitle('MicrosoftSoftware')/items";
    
    return clientBatch.get(url,SPHttpClientBatch.configurations.v1)
    .then(response => {
    return response.json();
    })
    .then(json => {
    ...
    public _addListItemsAsBatch(clientBatch: SPHttpClientBatch): Promise<ISoftwareListItem> {  
     ...
     clientBatch.post(url, SPHttpClientBatch.configurations.v1, spHttpClientOptions)
     .then((response: SPHttpClientResponse): Promise<ISoftwareListItem> => {
      return response.json();      
     })
